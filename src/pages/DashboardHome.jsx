...
const DashboardHome = () => {
  ...
  return (
    <div className="w-full p-6 flex gap-6 items-center justify-evenly flex-wrap">
      ...
      />
      <DashboardCard
        icon={<GiLoveSong className=" text-xl text-textColor" />}
        name="Songs"
        count={allSongs?.length > 0 ? allSongs.length : 0}
      />
      <DashboardCard
        icon={<RiUserStarFill className=" text-xl text-textColor" />}
        name="Artists"
        count={allArtists?.length > 0 ? allArtists.length : 0}
      />
      <DashboardCard
        icon={<GiMusicalNotes className=" text-xl text-textColor" />}
        name="Albums"
